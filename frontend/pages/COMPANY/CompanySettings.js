...
const getDraftJobHiring = async (authTokens, id) => {
  try {
    ...
    if (!res.ok) throw new Error("Failed to fetch job hiring details");
    return await res.json();
  } catch (error) {
    console.error("Error fetching job hiring details:", error);
    return null;
  }
};
...
export default function CompanySettings() {
  ...
  useEffect(() => {
    console.log("Stored Job Hiring ID:", id);

    const fetchJobHiringData = async () => {
      const jobHiringData = await getDraftJobHiring(authTokens, id);
      console.log("Fetched Job Hiring Data:", jobHiringData);
    };
            ...
            <div className="w-2/3 bg-white shadow-md rounded-lg p-6">
              ...
              <div className="mb-6">
                ...
                <div className="mb-4">
                  <div>
                    ...
                    >
                      <span>
                        {formData.criteria.workExperience.selectedOptions
                          .length > 0
                          ...
                          .map((option, index) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                value={option}
                                checked={formData.criteria.workExperience.selectedOptions.includes(
                                  option
                                )}
                                onChange={() =>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleCriteriaChange("skills", "weight", e.target.value)
                      }
                      className="w-16 border border-gray-300 rounded-lg px-2 py-1 text-sm text-fontcolor text-center"
                    />
                  </div>
                ...
                {[
                  {
                    label:
                      "Primary Skills (Put essential skills that are crucial for the job)",
                    key: "primary",
                  },
                  {
                    label:
                      "Secondary Skills (Put important skills that are frequently used but not absolutely essential)",
                    key: "secondary",
                  },
                  {
                    label:
                      "Additional Skills (Nice-to-have skills that would give a candidate an edge but aren’t necessary for the core job functions)",
                    key: "additional",
                  },
                ].map((skill, index) => (
              ...
              <div className="mb-6">
                ...
                {[
                  {
                    label:
                      "1st Choice Field of Study (Put the most directly relevant fields of study for the position)",
                    key: "firstChoice",
                  },
                  {
                    label:
                      "2nd Choice Field of Study (Put closely related fields that have significant overlap with the job requirements)",
                    key: "secondChoice",
                  },
                  {
                    label:
                      "3rd Choice Field of Study (Put fields that have some relevance or provide useful background knowledge)",
                    key: "thirdChoice",
                  },
                ].map((edu, index) => (
                  <div key={index} className="mb-4">
