...
class ResumeNER:
    def __init__(self, confidence_threshold=0.50):
        # Define labels
        self.LABELS = ['O', 'B-CERTIFICATION', 'I-CERTIFICATION', 'B-PROVIDER', 'I-PROVIDER']
    ...
    def _load_model_and_tokenizer(self):
        ...
        model.load_state_dict(torch.load(r'backend/main_model/certification_ner_model.pt', map_location=torch.device('cpu')))
        model.eval()
    ...
    def _get_entities_from_predictions(self, tokens, predictions, probabilities, offset=0, is_last_window=False):
        ...
        Returns:
            tuple: Lists of (certifications, providers) with their positions and confidence scores
        """
        certifications = []
        providers = []
        current_entity = []
            ...
            if token in ['[CLS]', '[SEP]', '[PAD]']:
                if current_entity:
                    ...
                    self._handle_current_entity(
                        ...
                        entity_start, offset, certifications, providers
                    )
                current_entity = []
    ...
    def _handle_current_entity(self, current_entity, current_probs, current_label,
        ...
        Args:
            ...
            certifications (list): List to store certification entities
            providers (list): List to store provider entities
        """
        ...
        if avg_confidence >= self.confidence_threshold:
            ...
            # Filtering of results for Certification
            if current_label.endswith('CERTIFICATION'):
                ...
                if (len(words) >= 2 and             # Must have at least 2 words
                    ...
                    if len(cleaned_text.split()) >= 2 and len(cleaned_text) >= 8:
                        certifications.append((cleaned_text, entity_start + offset, avg_confidence))

            # Filtering of results for Provider
            elif current_label.endswith('PROVIDER'):
                providers.append((entity_text, entity_start + offset, avg_confidence))

    def extract_entities(self, text):
        """
        Extract certifications and providers from the given text using sliding windows.

        Args:
            text (str): Input text containing certification information

        Returns:
            tuple: Two lists containing (certifications, providers) with confidence scores
        """
    ...
    def _process_single_window(self, text):
        ...
        Returns:
            tuple: Lists of (certifications, providers) with confidence scores
        """
        ...
        certifications, providers = self._get_entities_from_predictions(
            tokens['input_ids'][0],
            predicted_labels,
            token_probs,
            offset=0,
        ...
        return (
            [(title, conf) for title, _, conf in certifications],
            [(provider, conf) for provider, _, conf in providers]
        )
    ...
    def _process_sliding_windows(self, input_ids):
        ...
        for start_idx in range(0, total_length, self.window_size):
            ...
            window_entities.append((certifications, providers, start_idx, end_idx))

        # Merge entities with confidence scores
        merged_certifications = {}
        merged_providers = {}

        for certifications, providers, start_idx, end_idx in window_entities:
            # Only include entities that start in the non-overlapping region
            # except for the last window
            for title, pos, conf in certifications:
                if pos - start_idx < self.window_size or pos >= end_idx - self.stride:
                    # Keep entity with highest confidence if duplicated
                    if pos not in merged_certifications or conf > merged_certifications[pos][1]:
                        merged_certifications[pos] = (title, conf)

            for provider, pos, conf in providers:
                if pos - start_idx < self.window_size or pos >= end_idx - self.stride:
                    # Keep entity with highest confidence if duplicated
                    if pos not in merged_providers or conf > merged_providers[pos][1]:
                        merged_providers[pos] = (provider, conf)

        # Sort entities by position and return with confidence scores
        final_certifications = [item for _, item in sorted(merged_certifications.items())]
        final_providers = [item for _, item in sorted(merged_providers.items())]

        return final_certifications, final_providers


def ner_certification(text, confidence_threshold=0.50):
    """
    Process the input text and return extracted certification entities.
    ...
    Returns:
        tuple: Two lists containing:
            - certifications: List of tuples (title, confidence_score)
            - providers: List of tuples (provider, confidence_score)

    Example:
        >>> text = "I worked as a Software Engineer at Google"
        >>> certifications, providers = ner_certification(text, confidence_threshold)
...
if __name__ == "__main__":
    ...
    certifications, providers = ner_certification(input_text, confidence_threshold=0.50)

    # Print results with confidence scores
    print("\nExtracted Certifications:")
    for title, confidence in certifications:
        print(f"- {title} (confidence: {confidence:.2f})")

    print("\nExtracted Providers:")
    for provider, confidence in providers:
        print(f"- {provider} (confidence: {confidence:.2f})")