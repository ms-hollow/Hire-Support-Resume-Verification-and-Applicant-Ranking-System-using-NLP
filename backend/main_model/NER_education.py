...
class ResumeNER:
    def __init__(self, confidence_threshold=0.75):
        # Define labels
        self.LABELS = ['O', 'B-COURSE', 'I-COURSE', 'B-SCHOOL', 'I-SCHOOL']
    ...
    def _load_model_and_tokenizer(self):
        ...
        model.load_state_dict(torch.load(r'backend/main_model/education_ner_model.pt', map_location=torch.device('cpu')))
        model.eval()
    ...
    def _get_entities_from_predictions(self, tokens, predictions, probabilities, offset=0, is_last_window=False):
        ...
        Returns:
            tuple: Lists of (courses, schools) with their positions and confidence scores
        """
        courses = []
        schools = []
        current_entity = []
            ...
            if token in ['[CLS]', '[SEP]', '[PAD]']:
                if current_entity:
                    ...
                    self._handle_current_entity(
                        ...
                        entity_start, offset, courses, schools
                    )
                current_entity = []
    ...
    def _handle_current_entity(self, current_entity, current_probs, current_label,
        ...
        Args:
            ...
            courses (list): List to store course entities
            schools (list): List to store school entities
        """
        ...
        if avg_confidence >= self.confidence_threshold:
            ...
            # Filtering of results for Course
            if current_label.endswith('COURSE'):
                ...
                if (len(words) >= 2 and             # Must have at least 2 words
                    ...
                    if len(cleaned_text.split()) >= 2 and len(cleaned_text) >= 8:
                        courses.append((cleaned_text, entity_start + offset, avg_confidence))

            # Filtering of results for School
            elif current_label.endswith('SCHOOL'):
                schools.append((entity_text, entity_start + offset, avg_confidence))

    def extract_entities(self, text):
        """
        Extract courses and schools from the given text using sliding windows.

        Args:
            text (str): Input text containing education information

        Returns:
            tuple: Two lists containing (courses, schools) with confidence scores
        """
    ...
    def _process_single_window(self, text):
        ...
        Returns:
            tuple: Lists of (courses, schools) with confidence scores
        """
        ...
        courses, schools = self._get_entities_from_predictions(
            tokens['input_ids'][0],
            predicted_labels,
            token_probs,
            offset=0,
        ...
        return (
            [(title, conf) for title, _, conf in courses],
            [(school, conf) for school, _, conf in schools]
        )
    ...
    def _process_sliding_windows(self, input_ids):
        ...
        for start_idx in range(0, total_length, self.window_size):
            ...
            window_entities.append((courses, schools, start_idx, end_idx))

        # Merge entities with confidence scores
        merged_courses = {}
        merged_schools = {}

        for courses, schools, start_idx, end_idx in window_entities:
            # Only include entities that start in the non-overlapping region
            # except for the last window
            for title, pos, conf in courses:
                if pos - start_idx < self.window_size or pos >= end_idx - self.stride:
                    # Keep entity with highest confidence if duplicated
                    if pos not in merged_courses or conf > merged_courses[pos][1]:
                        merged_courses[pos] = (title, conf)

            for school, pos, conf in schools:
                if pos - start_idx < self.window_size or pos >= end_idx - self.stride:
                    # Keep entity with highest confidence if duplicated
                    if pos not in merged_schools or conf > merged_schools[pos][1]:
                        merged_schools[pos] = (school, conf)

        # Sort entities by position and return with confidence scores
        final_courses = [item for _, item in sorted(merged_courses.items())]
        final_schools = [item for _, item in sorted(merged_schools.items())]

        return final_courses, final_schools


def ner_education(text, confidence_threshold=0.75):
    """
    Process the input text and return extracted education entities.
    ...
    Returns:
        tuple: Two lists containing:
            - courses: List of tuples (title, confidence_score)
            - schools: List of tuples (school, confidence_score)

    Example:
        >>> text = "I worked as a Software Engineer at Google"
        >>> courses, schools = ner_education(text, confidence_threshold)
        >>> print(courses)
...
if __name__ == "__main__":
    ...
    courses, schools = ner_education(input_text, confidence_threshold=0.75)

    # Print results with confidence scores
    print("\nExtracted Courses:")
    for title, confidence in courses:
        print(f"- {title} (confidence: {confidence:.2f})")

    print("\nExtracted Schools:")
    for school, confidence in schools:
        print(f"- {school} (confidence: {confidence:.2f})")