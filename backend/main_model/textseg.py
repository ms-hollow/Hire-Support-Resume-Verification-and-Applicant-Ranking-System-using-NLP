...
def get_text_seg(frame, image):
    ...
    confThreshold = 0.4 # value to detect text regions
    ...
    # Check if indices list is empty before proceeding
    if len(indices) > 0:
        # If indices is a 1D array, we need to wrap it into a list of lists.
        ...
        for i in indices:
            ...
            for j in range(4):
                ...
                # Draw rectangle or lines as needed
                # cv2.line(frame, p1, p2, (0, 255, 0), 2, cv2.LINE_AA)
                # cv2.putText(frame, "{:.3f}".format(confidences[i[0]]), (vertices[0][0], vertices[0][1]),
                #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
            cv2.rectangle(mask, (min(x), min(y)), (max(x), max(y)), (255, 0, 0), -1)
    else:
        print("No bounding boxes detected or confidence threshold too high.")
